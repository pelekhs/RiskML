...
train_n_serve() {
    ...
    then
        for al in $3
        do
            mlflow run --experiment-name "$modelname" --entry-point train $curdir/.. -P task="$1" -P target="$2" -P train_size=1 -P n_folds=5 -P algo="$al" -P split_random_state=0 -P merge=1 --no-conda
        done
    fi
    if [[ $6 != "--no-serving" ]]
    then
